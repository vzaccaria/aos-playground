...
void clear_readwrite_fds(fd_set *readfds_master, fd_set *writefds_master) {
  FD_ZERO(&readfds_master);
  FD_ZERO(&writefds_master);
}

void update_readwrite_fds_with_state(fd_set *readfds_master,
                                     fd_set *writefds_master, int fd,
                                     fd_status_t status) {
  if (status.want_read) {
...
int main(int argc, char **argv) {
  ...
  clear_readwrite_fds(&readfds_master, &writefds_master);
    ...
    for (int fd = 0; fd <= fdset_max && nready > 0; fd++) {
      ...
      if (FD_ISSET(fd, &readfds)) {
        ...
        if (fd == listener_sockfd) {
          ...
          status = on_peer_connected(newsockfd, &peer_addr, peer_addr_len);
          fd = newsockfd;
        } else {
          status = on_peer_ready_recv(fd);
        }
        update_readwrite_fds_with_state(&readfds_master, &writefds_master, fd,
                                        status);
      }
