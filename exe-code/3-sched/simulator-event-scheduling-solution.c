...
task_t *SRTF(task_t *ready_queue[], unsigned int nr_ready_queue, int t) {

    int min_idx=-1;
    unsigned int i, min_C = INT_MAX;
    for(i=0; i<nr_ready_queue; i++) {
        if (ready_queue[i]->remaining_time < min_C) {
            min_C = ready_queue[i]->remaining_time;
            min_idx = i;
        }
    }
...
task_t *HRRN(task_t *ready_queue[], unsigned int nr_ready_queue, int t) {

    if (running_task != NULL) {
        // HRRN is non-preemptive, we cannot preempt the task
        return running_task;
    }
    
    int max_idx=-1, max_RR = -1;
    unsigned int i;
    for(i=0; i<nr_ready_queue; i++) {
        int RR = ((t - ready_queue[i]->a_time + ready_queue[i]->C_time) / (float)ready_queue[i]->C_time);
        if (RR > max_RR) {
            max_RR = RR;
            max_idx = i;
        }
    ...
    if (max_idx == -1) {
        ...
        return ready_queue[max_idx];
    }
}

task_t *RR(task_t *ready_queue[], unsigned int nr_ready_queue, int t) {

    static int curr_quantum = 0;
    unsigned int i;
    ...
    if (running_task != NULL) {
        if (--curr_quantum > 0) {
            ...
            for(i=0; i<nr_ready_queue; i++) {
                if (ready_queue[i]->a_time > max_a) {
                    max_a = ready_queue[i]->a_time;
                }
            }
            running_task->a_time = max_a+1;
        }
    }

    int min_idx=-1;
    unsigned int min_a = INT_MAX;
    for(i=0; i<nr_ready_queue; i++) {
        if (ready_queue[i]->a_time < min_a) {
            min_a = ready_queue[i]->a_time;
            min_idx = i;
        }
    }
